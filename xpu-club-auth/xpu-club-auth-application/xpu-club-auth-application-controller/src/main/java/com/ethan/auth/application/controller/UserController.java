package com.ethan.auth.application.controller;

import cn.dev33.satoken.stp.SaTokenInfo;
import cn.dev33.satoken.stp.StpUtil;
import com.alibaba.fastjson.JSON;
import com.google.common.base.Preconditions;
import com.ethan.auth.application.convert.AuthUserDTOConverter;
...
public class UserController {
    ...
    public Result<Boolean> delete(@RequestBody AuthUserDTO authUserDTO) {
        try {
            ...
            AuthUserBO authUserBO = AuthUserDTOConverter.INSTANCE.convertDTOToBO(authUserDTO);
            return Result.ok(authUserDomainService.deleteById(authUserBO));  // 这里直接调用更新接口，因为删除接口没有必要，而且删除接口会导致用户无法登录，所以直接调用更新接口
        } catch (Exception e) {
            log.error("UserController.update.error:{}", e.getMessage(), e);
